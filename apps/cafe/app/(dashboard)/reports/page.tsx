        ...
        <CardContent className="flex flex-wrap items-end gap-3 p-4">
          ...
          <Button
            ...
            onClick={() => {
              ...
              exportToCSV(
                buildReportCsvRows(data, range),
                `report-${range.startDate}-to-${range.endDate}`,
              );
            }}
          >
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <SummaryCard
              ...
              loading={report.isPending}
            />
            {/* Money taken in-range against a PRE-EXISTING due (CR1.4) —
                separate from "Collected" above (order-paidAmount-only), never
                merged into it (see OrderSummary.duesCollected). Label carries
                "(this range)" because the Customer dues table below is a
                LIVE, unbounded snapshot (G12) — the two numbers do not share
                a time basis and must never be read as one figure. */}
            <SummaryCard
              title="Dues collected (this range)"
              value={inr(totals?.duesCollected ?? 0)}
              icon={Banknote}
              variant="red"
              loading={report.isPending}
            />
